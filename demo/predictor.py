...
class VisualizationDemo(object):
    def __init__(self, cfg, instance_mode=ColorMode.IMAGE, parallel=False):
        ...
        if parallel:
            ...
            self.predictor = DPTEXTPredictor(cfg)

    def run_on_image(self, image, texts, texts_number):
        """
        Args:
        ...
        """
        vis_output = None
        predictions = self.predictor(image, texts, texts_number)
...
class DPTEXTPredictor(DefaultPredictor):
    """
        heeh
    """

    def __init__(self, cfg):
        super(DPTEXTPredictor, self).__init__(cfg)

    def __call__(self, original_image, texts, texts_number):
        """
            (...)
        """
        with torch.no_grad():  # https://github.com/sphinx-doc/sphinx/issues/4258
            ...
            predictions = self.model.forward_query([inputs], texts, texts_number)[0]
            return predictions
...
class AsyncPredictor:
    ...
    class _PredictWorker(mp.Process):
        ...
        def run(self):
            predictor = DPTEXTPredictor(self.cfg)
