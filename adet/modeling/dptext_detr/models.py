...
from adet.layers.label_encoder import LabelEncoder
...
class DPText_DETR(nn.Module):
    def __init__(self, cfg, backbone):
        ...
        self.points_num_classes = 2 # text but not target/target text/no text
        ...
        self.emb_dim = cfg.MODEL.LABELENCODER.EMB_DIM
        self.n_trans_layers = cfg.MODEL.LABELENCODER.N_TRANS_LAYERS
        self.label_ckpt_path = cfg.MODEL.LABELENCODER.WEIGHT
        self.max_text_len = cfg.MODEL.LABELENCODER.MAX_LEN
        ...
        self.label_encoder = LabelEncoder(
            max_len=self.max_text_len, emb_dim=self.emb_dim, n_heads=self.nhead, 
            n_trans_layers=self.n_trans_layers, ckpt_path=self.label_ckpt_path
        )
        ...
        self.ctrl_point_class = nn.Linear(self.d_model, self.points_num_classes)
        self.ctrl_point_coord = MLP(self.d_model, self.d_model, 2, 3)
        # self.ctrl_point_text = nn.Linear(self.d_model, self.voc_size + 1)
        ...
        self.ctrl_point_class.bias.data = torch.ones(self.points_num_classes) * bias_value
        ...
        # self.ctrl_point_text = nn.ModuleList([self.ctrl_point_text for _ in range(num_pred)])
        if self.epqm:
    ...
    def forward(self, x: tuple):
        ...
        with torch.no_grad():
            text_encoding = self.label_encoder(x[1])
        queries = text_encoding.unsqueeze(1).repeat(1, self.num_proposals, 1, 1)
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact = self.transformer(
            srcs, masks, pos, ctrl_point_embed, queries, query_pos_embed
        )
        ...
        if self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord) # outputs_text,
