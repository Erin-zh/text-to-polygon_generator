...
class RandomRotate(Augmentation):
    def __init__(self, p, rotate_limit):
        """
        Args:
            p (float): probability
        """
    ...
    def get_transform(self):
        return RandomRotateTransform(self.p, self.rotate_limit)
    
class RandomRotateTransform(Transform):
    def __init__(self, p, rotate_limit):
        super().__init__()
        self.random_rotate = Rotate(p=p,limit=rotate_limit)
        self._set_attributes(locals())

    def apply_image(self, img: np.ndarray):
        return self.random_rotate(image=img)['image']
...
class RandomFlip(Augmentation):
    ...
    def get_transform(self, img):
        return RandomHorizonTransform(self.p)
    
class RandomHorizonTransform(Transform):
    def __init__(self, p):
        super().__init__()
        self.random_roate = HorizontalFlip(p=p)
        self._set_attributes(locals())

    def apply_image(self, img: np.ndarray):
        return self.random_roate(image=img)['image']
