...
from .augmentation import RandomBlur, GaussNoise, RandomHueSaturationValue, RandomRotate, RandomFlip
import random
...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    if "text_number" in annos[0]:
        text_number = [obj.get("text_number", []) for obj in annos]
        instance.text_number = torch.as_tensor(text_number, dtype=torch.int32)
...
def build_augmentation(cfg, is_train):
    ...
    if is_train:
        ...
        augmentation.append(RandomRotate(0.5, 30))
        # augmentation.append(RandomFlip(0.5))
        # augmentation.append(RandomWarpAugmentation(random.random()))
