...
from adet.utils.remove_duplicates import remove_duplicates_within_image
...
class TextDetEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        elif "totaltext_poly_test" in dataset_name:
            self._text_eval_gt_path = "datasets/evaluation/gt_totaltext.zip"
        elif "ctw1500_word_poly_test" in dataset_name:
            self._text_eval_gt_path = "datasets/evaluation/gt_ctw1500.zip"
        elif "icdar2015_test" in dataset_name :
    ...
    def process(self, inputs, outputs):
        for input, output in zip(inputs, outputs):
            ...
            prediction["instances"] = self.instances_to_coco_json(instances, input["image_id"], input["file_name"], input['current_annotation'][0]['text'])
    ...
    def evaluate(self):
        ...
        if self.submit:
            ...
            unique_coco_results = remove_duplicates_within_image(coco_results)
            file_path = os.path.join(self._output_dir, "text_results.json")

        self._logger.info("Saving results to {}".format(file_path))
        with PathManager.open(file_path, "w") as f:
            f.write(json.dumps(unique_coco_results))
            f.flush()
    ...
    def instances_to_coco_json(self, instances, img_id, img_name, text):
        ...
        # max_score_index = np.argmax(scores).item()
        # instance = instances[max_score_index]
