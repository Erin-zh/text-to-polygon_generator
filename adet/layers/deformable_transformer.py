...
class DeformableTransformer_Det(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed, queries, query_pos_embed):
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, query, padding_mask=None):
        # self attention
        src2 = self.self_attn(
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None): # queries,
        output = src
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device)
        for _, layer in enumerate(self.layers):
            output = layer(output, pos, reference_points, spatial_shapes, level_start_index, padding_mask) # queries,
